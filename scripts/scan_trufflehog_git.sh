...
OUTPUT_FILE="${REPO_ANALYSIS_DIR}/trufflehog_git_scan_results.json"
LOG_FILE="${REPO_ANALYSIS_DIR}/trufflehog_git_scan.log"

echo "--- Starting TruffleHog Git Scan for Repo: ${REPO_NAME} ---" | tee "$LOG_FILE"
...
# Run TruffleHog on the current Git repository
...
trufflehog git file://. \
    ...
    || { echo "Error: TruffleHog Git scan failed. Check logs." | tee -a "$LOG_FILE"; exit 1; }

echo "--- Finished TruffleHog Git Scan for Repo: ${REPO_NAME} ---" | tee -a "$LOG_FILE"
echo "----------------------------------------------------------" | tee -a "$LOG_FILE"
